...
(define-word reload (:word "RELOAD")
  ...
  (let ((name (word files #\Space)))
    (when (null name)
      (forth-exception :zero-length-name))
    (let* ((word-list (dictionary-name (word-lists-compilation-word-list word-lists)))
           (key (format nil "~A.~A" word-list name))
           (entry (gethash key *predefined-words*))
           (new-word (cdr entry)))
      (when (null new-word)
        (forth-exception :undefined-word "~A is not a predefined word in ~A" name word-list))

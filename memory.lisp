...
(deftype forth-address () 'fixnum)
(deftype forth-memory () '(simple-array (unsigned-byte 8) (*)))
...
(defun make-address (prefix address)
  (declare (type (integer 0 #xFF) prefix) (type forth-address address)
...
(defun address-prefix (address)
  (declare (type forth-address address) (optimize (speed 3) (safety 0)))
...
(defmacro dpbs (&rest values-and-fields)
  ...
  (loop with f-and-v = (reverse values-and-fields)
	with form = (pop f-and-v) finally (return form)
	for (field value) on f-and-v by #'cddr
	do (setf form `(dpb ,value ,field ,form))))
...
(defmethod cell-at ((sp data-space) address)
  (with-slots (data size) sp
    ...
    (let ((base-address (ash (ash address +byte-to-cell-shift+) (- +byte-to-cell-shift+)))
          (data data))
      (declare (type forth-address base-address)
               (type forth-memory data)
               #.+forth-optimize-settings+)
      (cell-signed (dpbs (aref data (the fixnum (+ base-address 7))) (byte 8 56)
                         (aref data (the fixnum (+ base-address 6))) (byte 8 48)
                         (aref data (the fixnum (+ base-address 5))) (byte 8 40)
                         (aref data (the fixnum (+ base-address 4))) (byte 8 32)
                         (aref data (the fixnum (+ base-address 3))) (byte 8 24)
                         (aref data (the fixnum (+ base-address 2))) (byte 8 16)
                         (aref data (the fixnum (+ base-address 1))) (byte 8  8)
  ...
  (with-slots (data size) sp
    ...
    (let ((base-address (ash (ash address +byte-to-cell-shift+) (- +byte-to-cell-shift+))))
      (declare (type forth-address base-address)
               (type forth-memory data)
               #.+forth-optimize-settings+)
      (dpbs (aref data (+ base-address 7)) (byte 8 56)
            (aref data (+ base-address 6)) (byte 8 48)
            (aref data (+ base-address 5)) (byte 8 40)
            (aref data (+ base-address 4)) (byte 8 32)
            (aref data (+ base-address 3)) (byte 8 24)
            (aref data (+ base-address 2)) (byte 8 16)
            (aref data (+ base-address 1)) (byte 8  8)
            (aref data base-address)))))
  ...
  (with-slots (data size) sp
    ...
    (let ((base-address (ash (ash address +byte-to-quad-byte-shift+) (- +byte-to-quad-byte-shift+)))
      ...
      (declare (type forth-address base-address)
               (type forth-memory data)
               #.+forth-optimize-settings+)
      (quad-byte-signed (dpbs (aref data (+ base-address 3)) (byte 8 24)
                              (aref data (+ base-address 2)) (byte 8 16)
                              (aref data (+ base-address 1)) (byte 8  8)
                              (aref data base-address))))))
...
(defun address-space (memory address)
  (declare (type memory memory) (type forth-address address)
           (optimize (speed 3) (safety 0)))
...
(define-memory-fun native-address (memory foreign-pointer)
  (let ((foreign-address (pointer-address foreign-pointer)))
    ...
    (forth-exception :invalid-memory "Can't map foreign address $~16,'0X to a Forth address" foreign-address)))
